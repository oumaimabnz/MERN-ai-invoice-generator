            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
                {REVIEWS.map((review, index) => (
                    <div key={index} className='bg-gray-50 rounded-2xl p-8 shadow-sm hover:shadow-lg transition-all duration-300 relative'>
                        ...
                        <p className='text-gray-700 mb-6 leading-relaxed italic text-lg'>"{review.quote}"</p>
                        <div className='flex items-center space-x-4'>
                            <img src={review.avatar} alt={review.author} className='w-12 h-12 rounded-full object-cover border-2 border-gray-100'/>
                            <div className='flex-1'>
                                <p className='font-semibold text-gray-900'>{review.avatar}</p>
