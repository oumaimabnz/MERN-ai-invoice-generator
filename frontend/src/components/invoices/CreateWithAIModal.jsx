...
const CreateWithAIModal = ({isOpen, onClose}) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const invoiceDate = response.data

            toast.success('Invoice data extracted successfully!')
            onClose()

            navigate('/invoices/new', { state: { aiData: invoiceDate}})
        } catch (error) {
        ...
        <div className='flex items-center justify-center min-h-screen px-4 text-center'>
            ...
            <div className='bg-white rounded-lg shadow-xl max-w-lg w-full p-6 relative text-left transform transition-all'>
                <div className='flex items-center justify-between mb-4'>
                   ...
                   <button onClick={onClose} className='tex-slate-400 hover:text-slate-600'>&times;</button>
               </div>
