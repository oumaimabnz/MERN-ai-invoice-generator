...
const CreateInvoice = ({existingInvoice, onSave}) => {
  ...
  useEffect(() => {
   if (!user || existingInvoice) return
  ...
  return <form onSubmit={handleSubmit} className='space-y-8 pb-[100vh]'>
    ...
    <div className='bg-white border border-slate-200 rounded-lg shadow-sm shadow-gray-100 overflow-hidden'>
        ...
        <table className='w-full divide-y divide-slate-200'>
          <thead className='bg-slate-50'>
            <tr>
              ...
              <th className='px-2 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider'>Totam</th>
      ...
      <div className='p-4 sm:p-6 border-t border-slate-200'>
        <Button type='button' variant='secondary'onClick={handleAddItems} icon={Plus}>Add Item</Button>
      </div>
    </div>

    <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
      <div className='bg-white p-6 rounded-lg shadow-sm shadow-gray-100 border border-slate-200 space-y-4'>
        <h3 className='lext-lg font-semibold text-slate-900 mb-2'>Notes & Terms</h3>
