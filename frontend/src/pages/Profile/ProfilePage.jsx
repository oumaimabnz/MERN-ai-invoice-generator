...
const ProfilePage = () => {
  ...
  const handleUpdaeProfile = async (e) => {
    e.preventDefault()
    setIsUpdating(true)
    try {
  ...
  return (
    <div className='bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden max-w-4xl mx-auto'>
      ...
      <form onSubmit={handleUpdaeProfile}>
        <div className='p-6 space-y-6'>
          ...
          <InputField label="Full Name" name="name" icon={User} type='text' value={formData.name} onChange={handleInputChange} placeholderr="Enter your full name"/>

          <div className='pt-6 border-t border-slate-200'>
            <h4 className='text-lg font-medium text-slate-900'>Business Information</h4>
            <p className='text-sm text-slate-500 mt-1 mb-4'>This will be used to pre-fill the "Bill From" section of your invoice</p>
            <div className='space-y-4'>
              <InputField label="Business Name" name="businessName" icon={Building} type='text' value={formData.businessName} onChange={handleInputChange} placeholderr="Your Company name"/>
              <TextareaField label='Address' name='address' icon={MapPin} value={formData.address} onChange={handleInputChange} placeholderr='123 Main Str, Chemnitz, Germany'/>
              <InputField label="Phone" name="phone" icon={Phone} type='tel' value={formData.phone} onChange={handleInputChange} placeholderr="(0) 152 1234-5678"/>
            </div>
          </div>
        </div>
